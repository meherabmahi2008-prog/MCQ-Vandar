...
class _AdminPageState extends State<AdminPage> with SingleTickerProviderStateMixin {
  ...
  // মেমরিতে ধরে রাখার জন্য বিষয়, বোর্ড ও সাল
  ...
  // --- [একসাথে একাধিক প্রশ্ন নির্বাচন করে ডিলিট করার জন্য] ---
  ...
  void _verifyPassword() {
    if (_passwordController.text == _appPassword) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('ভুল পাসওয়ার্ড! আবার চেষ্টা করুন।'),
  ...
  Future<void> _uploadQuestion() async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('প্রশ্ন সফলভাবে আপলোড হয়েছে! 🎉'),
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('ভুল হয়েছে: $e'), backgroundColor: Colors.red),
        );
      }
    } finally {
  ...
  Future<void> _fetchQuestions() async {
    ...
    try {
      ...
      setState(() {
        ...
        _selectedIds.clear(); // নতুন সার্চে আগের নির্বাচন মুছে যাবে
      });
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('সার্চে সমস্যা হয়েছে: $e')),
        );
      }
    } finally {
  ...
  // --- [একসাথে নির্বাচিত একাধিক প্রশ্ন ডিলেট করার লজিক] ---
  Future<void> _deleteSelectedQuestions() async {
    ...
    try {
      await Future.wait(
        idsToDelete.map(
          (id) => Supabase.instance.client.from('questions').delete().eq('id', id),
        ),
      );
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('${idsToDelete.length}টি প্রশ্ন মুছে ফেলা হয়েছে!'),
            backgroundColor: Colors.redAccent,
          ),
        );
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('বাল্ক ডিলিট করতে সমস্যা: $e')),
        );
      }
    }
  }

  // --- [নির্বাচিত প্রশ্নগুলো ডিলিট করার আগে নিশ্চিতকরণ পপআপ] ---
  void _confirmDeleteSelected() {
    showDialog(
      context: context,
      builder: (c) => AlertDialog(
        backgroundColor: const Color(0xFF2C2C2C),
        title: const Text('নিশ্চিত?', style: TextStyle(color: Colors.white)),
        content: Text(
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            child: const Text('হ্যাঁ, সবগুলো ডিলিট করুন', style: TextStyle(color: Colors.white)),
          ),
        ],
      ),
    );
  ...
  // --- [প্রশ্ন এডিট/আপডেট করার ডায়ালগ] ---
                      ...
                      children: [
                        Expanded(
                          child: TextField(
                            ...
                            style: const TextStyle(color: Colors.white),
                            decoration: const InputDecoration(labelText: 'বিষয়', labelStyle: TextStyle(color: Colors.white70)),
                          ),
                        ),
      ...
      builder: (context) => AlertDialog(
        ...
        content: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 10),
              Text('💡 ব্যাখ্যা: ${q['explanation'] ?? 'কোনো ব্যাখ্যা দেওয়া হয়নি'}', style: const TextStyle(color: Colors.white70)),
              const SizedBox(height: 8),
              Text('📚 বিষয়: ${q['subject'] ?? 'N/A'}', style: const TextStyle(color: Colors.white70)),
        ...
        actions: [
          ...
          TextButton(
            ...
            child: const Text('পরিবর্তন করুন', style: TextStyle(color: Colors.amber)),
          ),
          TextButton(
            onPressed: () {
              showDialog(
            ...
            child: const Text('ডিলিট করুন', style: TextStyle(color: Colors.redAccent)),
          ),
        ],
      ),
    );
            ...
            children: [
              Expanded(
                child: TextField(
                  ...
                  style: const TextStyle(color: Colors.white),
                  decoration: _inputDecoration('বিষয়'),
                ),
              ),
      ...
      child: Column(
        children: [
          TextField(
            ...
            style: const TextStyle(color: Colors.white),
            decoration: _inputDecoration('প্রশ্ন দিয়ে খুঁজুন', icon: Icons.search),
          ),
          const SizedBox(height: 8),
          Row(
            children: [
              Expanded(
                child: TextField(
                  ...
                  style: const TextStyle(color: Colors.white),
                  decoration: _inputDecoration('বিষয় ফিল্টার'),
                ),
              ),
              const SizedBox(width: 8),
              Expanded(
          ...
          // --- [সব নির্বাচন করুন + নির্বাচিতগুলো ডিলিট করুন বার] ---
          if (_searchResults.isNotEmpty)
            Row(
              children: [
                Checkbox(
                  value: _selectedIds.length == _searchResults.length,
                  activeColor: Colors.redAccent,
                  onChanged: (checked) {
                    setState(() {
                      if (checked == true) {
                        _selectedIds
                          ..clear()
                          ..addAll(_searchResults.map((q) => (q['id'] as num).toInt()));
                      } else {
                        _selectedIds.clear();
                      }
                    });
                ...
                Text(
                  _selectedIds.isEmpty
                      ? 'সব নির্বাচন করুন'
                      : '${_selectedIds.length}টি নির্বাচিত',
                  style: const TextStyle(color: Colors.white70),
                ),
                const Spacer(),
          ...
          Expanded(
            child: _isSearching
                ...
                : _searchResults.isEmpty
                    ? const Center(child: Text('কোনো প্রশ্ন পাওয়া যায়নি', style: TextStyle(color: Colors.white54)))
                    : ListView.builder(
                        itemCount: _searchResults.length,
                        itemBuilder: (context, index) {
                          final q = _searchResults[index];
                          final qId = (q['id'] as num).toInt();
                          final isSelected = _selectedIds.contains(qId);
                          return Card(
